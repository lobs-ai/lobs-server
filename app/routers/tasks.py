...
async def create_task(
    ...
    # Classify model tier synchronously so the response includes the tier.
    # Caller-provided tier wins; default to "standard" if classification fails.
    if not db_task.model_tier:
        try:
            tier = await classify_task_tier(db_task, db)
            db_task.model_tier = tier
        except Exception as exc:
            logger.warning("[TASKS] Tier classification failed for %s, defaulting to standard: %s", db_task.id, exc)
