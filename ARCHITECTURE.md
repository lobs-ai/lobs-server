...
- **First-Failure Evidence Bundle** — Deterministic failure packet generated once per failed task. New `failure_bundles` table (`UNIQUE` on `task_id`) captures: state transition timeline, triggering exception, first failing command, cancellation reason, error type, and run metrics at the moment of first failure. `FailureBundleBuilder.capture()` in `app/services/failure_bundle.py` is called from `worker._handle_completion()` on failure — idempotent, first-failure-only (second call returns existing ID without modifying the row). Bundle ID is persisted to `Task.failure_bundle_id` and injected into diagnostic prompts via `diagnostic_triggers._build_prompt()` so diagnostic agents consume pre-computed context rather than re-deriving it. `FailureExplainerService` includes bundle in `supporting_data`. Design: [docs/failure-evidence-bundle-design.md](docs/failure-evidence-bundle-design.md). Handoffs: [docs/handoffs/failure-evidence-bundle-handoffs.json](docs/handoffs/failure-evidence-bundle-handoffs.json).

- **Goal-to-Plan Task Decomposition** — Two-phase API that converts a natural-language objective into a structured execution plan (parent task + ordered subtasks + suggested agent owners). `POST /api/tasks/decompose` calls LLM (standard-tier via ModelChooser/Gateway sessions_spawn), stores `TaskDecomposition` record with raw LLM output, returns preview. `POST /api/tasks/decompose/{id}/commit` creates parent Task + subtasks with `Task.parent_task_id` FK and `Task.subtask_order` set; converts `depends_on` indices to `Task.blocked_by` task IDs. `DELETE /api/tasks/decompose/{id}` discards plan (learning signal). New service: `app/services/decomposition.py`. New router: `app/routers/decompose.py`. New DB: `task_decompositions` table + `parent_task_id`/`subtask_order` columns on `tasks`. Design: [docs/goal-to-plan-decomposition-design.md](docs/goal-to-plan-decomposition-design.md). Handoffs: [docs/handoffs/goal-to-plan-decomposition-handoffs.json](docs/handoffs/goal-to-plan-decomposition-handoffs.json).
- **60-Second Decision Card** — Standard format for blocked-task human approvals. Three urgency tiers (🔴 Critical 2h / 🟡 Standard 24h / 🟢 Advisory 72h) with required fields: deadline, options+tradeoffs, recommendation, consequence of no response. Delivered via inbox items. Spec: [docs/communication/decision-card-spec.md](docs/communication/decision-card-spec.md). Sample copy with 5 real examples: [docs/communication/decision-card-copy.md](docs/communication/decision-card-copy.md). 3 programmer handoffs: utility module (`app/utils/decision_card.py`), inbox schema extension (`DecisionCardPayload`), BriefService `PendingDecisionsAdapter`.
